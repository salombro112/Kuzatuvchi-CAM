...
/* Ism/telefon kiritilgach: qaysi tasdiqlash yo'li tanlanishi tartibi:
   1) Admin login (maxsus "parol") — hech qanday tasdiqlash shart emas.
   2) Telegram Mini App ichida ochilgan bo'lsa — Telegram'ning o'zi
      taqdim etadigan "telefon raqamni ulashish" (requestContact) orqali,
      SMS'siz, bepul va tasdiqlangan raqam bilan kiritiladi.
   3) SMS xizmati sozlangan bo'lsa (CONFIG.SMS.API_KEY to'ldirilgan) —
      4 xonali kod SMS orqali yuboriladi.
   4) Aks holda (na Telegram, na SMS) — tasdiqlashsiz to'g'ridan-to'g'ri
      kiritiladi, shunda do'kon SMS sozlanmagan holatda ham ishlayveradi. */
...
/* SMS API kaliti sozlanganmi (hali ham "YOUR_ISMS_API_KEY" bo'lsa —
   sozlanmagan hisoblanadi). */
function isSmsConfigured() {
  return !!(CONFIG.SMS.API_KEY && CONFIG.SMS.API_KEY !== "YOUR_ISMS_API_KEY");
}
...
/* Ilova Telegram bot ichida (Mini App sifatida) ochilganda true bo'ladi —
   shunda telefon raqami SMS o'rniga Telegram'ning o'z tasdiqlangan
   "requestContact" funksiyasi orqali olinadi (bepul, tez, ishonchli). */
...
function initTelegramWebApp() {
  if (window.Telegram && window.Telegram.WebApp) {
    ...
    if (typeof tg.requestContact === "function") {
      ...
      // Botdan kirilganda telefonni qo'lda kiritish shart emas (Telegram
      // o'zi so'raydi) — lekin maydonni YASHIRMAYMIZ, chunki admin ham
      // shu formadan, maxsus "parol"ini shu maydonga kiritib kiradi.
      // Shunchaki ixtiyoriy qilib qo'yamiz va izohini yangilaymiz.
      const phoneInput = document.getElementById("regPhone");
      if (phoneInput) {
        phoneInput.removeAttribute("required");
        phoneInput.placeholder = "Admin bo'lsangiz shu yerga kod kiriting";
      }
      const submitBtn = document.querySelector("#onboardForm button[type=submit]");
      if (submitBtn) submitBtn.textContent = "Telefon raqamni ulashish";
      const sub = document.querySelector("#onboarding .onboard-sub");
      if (sub) sub.textContent = "Davom etish uchun Telegram orqali telefon raqamingizni ulashing — bu xavfsiz va bir necha soniya vaqt oladi.";
    }
  }
}

/* Telegram'ning native "telefon raqamni ulashish" oynasini ochadi.
   Foydalanuvchi rozi bo'lsa, Telegram allaqachon tasdiqlagan haqiqiy
   raqam qaytariladi — qo'shimcha SMS tasdiqlash shart emas. */
function requestTelegramPhone(name) {
  const submitBtn = document.querySelector("#onboardForm button[type=submit]");
  if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = "Kutilmoqda..."; }

  window.Telegram.WebApp.requestContact((success, info) => {
    if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = "Telefon raqamni ulashish"; }
    const contact = info && info.responseUnsafe && info.responseUnsafe.contact;
    if (!success || !contact || !contact.phone_number) {
      showToast("Telefon raqami ulashilmadi. Davom etish uchun ruxsat bering.");
      return;
    }
    completeLogin(name, contact.phone_number, false);
  });
}
