...
// shu raqamni ulashgan foydalanuvchi admin bo'ladi
const ADMIN_PHONE = "+998330000746";

// raqamlarni solishtirish uchun faqat raqamlarni qoldiradi (bo'shliq, + va h.k. olib tashlanadi)
...
// akkauntni Firebase "accounts" ro'yxatiga yozadi (Admin > Akkauntlar uchun)
...
// login sayt orqali emas, Telegram bot orqali (kontakt ulashish) amalga oshadi
...
// Telegram user id bo'yicha Firebase'dan kontaktni o'qib, avtomatik kirish
// SDK (websocket) o'rniga REST (fetch) orqali — Telegram WebView'da websocket
// ko'pincha ishlamay, doim timeoutga uchraydi
function resolveTelegramAccount(tgUserId) {
  ...
  const url = `${CONFIG.FIREBASE.databaseURL}/telegramContacts/${tgUserId}.json`;
  ...
  fetch(url, { signal: controller.signal })
    ...
    .catch((err) => {
      ...
      console.error(isTimeout ? "So'rov vaqti tugadi (timeout)" : "Telegram kontaktini o'qishda xatolik:", err);
      showOnboardingFallback(isTimeout ? "timeout" : "error");
    });
}

// isAdmin har safar Firebase'dan yangi o'qilgan qiymatga qo'yiladi, eski holat bilan qo'shilmaydi
function completeLogin(name, phone, isTheAdmin) {
  ...
  saveJSON(LS.knownAccounts, state.knownAccounts);
  syncAccountToCloud(state.account);
  // boot() chaqirmaymiz — aks holda cheksiz tsikl bo'ladi
...
// input fokusini yo'qotmasdan kabel summalarini DOM'da yangilaydi
...
function attachDynamicListeners() {
  ...
  // render() chaqirmaymiz, aks holda input fokusdan chiqib qoladi
...
// joylashuvni aniqlab pendingLocation ga yozadi, signal zaif bo'lsa kamroq aniqlik bilan qayta urinadi
...
function boot() {
  // Telegram ichida bo'lsak har doim Firebase'dan qayta tekshiramiz,
  // localStorage'dagi eski akkauntga (va uning admin holatiga) ishonmaymiz
