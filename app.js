const CONFIG = {
  BOT_TOKEN: "8956590718:AAH595jK6PUdTpmjh9Ozdv4VUDgREgMPo5o",     // <-- @BotFather'dan olingan token (haqiqiy bot bilan bir xil bo'lishi shart!)
...
/* Telefon raqamlarni solishtirish uchun faqat raqamlarni qoldiradi
   (bo'sh joy, "+", tire va h.k. olib tashlanadi) — Telegram ba'zan
   raqamni "+998 33 000 07 46" kabi bo'shliqlar bilan qaytarishi mumkin,
   bu esa aniq (qat'iy) satr solishtirishda mos kelmay qolib, admin
   noto'g'ri aniqlanishiga sabab bo'lardi. */
function normalizePhone(p) {
  return String(p || "").replace(/\D/g, "");
}
const ADMIN_PHONE_DIGITS = normalizePhone(ADMIN_PHONE);

/* ---------------------------- Mahsulotlar katalogi (standart) ---------------------------- */
const DEFAULT_PRODUCTS = (typeof PRODUCTS_DATA !== "undefined") ? PRODUCTS_DATA : [];
/* Boshlang'ich kategoriyalar ro'yxati (birinchi marta ishga tushganda
   yuklanadi). Admin bu ro'yxatni saytdan turib tartibini o'zgartirishi
   va yangi kategoriya qo'shishi mumkin — o'zgarishlar getAllCategories()
   orqali qo'llaniladi (pastda). */
...
const LS = {
  ...
  categoryOverrides: "kc_category_overrides", // demo-rejim: {slug: {...changed fields (order/label/icon), deleted?:true}}
  customCategories: "kc_custom_categories",   // demo-rejim: [ {slug,label,icon,order, deleted?:true} ]
  ratings: "kc_ratings",                      // demo-rejim: {mahsulot_id: {telefon: 1..5}}
};
...
let state = {
  ...
  // Umumiy (Firebase) kategoriyalar ro'yxati — sinxronlashgach shu to'ldiriladi.
  cloudCategories: null,
  categoryOverrides: loadJSON(LS.categoryOverrides, {}),
  customCategories: loadJSON(LS.customCategories, []),
  // Umumiy (Firebase) baholar ro'yxati — sinxronlashgach shu to'ldiriladi.
  cloudRatings: null,
  ratings: loadJSON(LS.ratings, {}),
  screen: "home",
  category: "all",
  searchQuery: "",
};
...
function persist() {
  ...
  if (!FIREBASE_ENABLED) {
    ...
    saveJSON(LS.ratings, state.ratings);
  }
}
...
function initFirebase() {
  ...
  try {
    ...
    // Kategoriyalar: admin qo'shgan/tartibini o'zgartirgan kategoriyalarni
    // BARCHA foydalanuvchiga bir zumda yetkazish uchun.
    ...
    // Baholar: mahsulotlarga qo'yilgan yulduzcha baholar (1-5) — BARCHA
    // foydalanuvchilar uchun real-time. Struktura: ratings/<mahsulot_id>/<telefon> = 1..5
    fbDb.ref("ratings").on("value", (snap) => {
      state.cloudRatings = snap.val() || {};
      render();
    });
  } catch (e) {
...
/* Joriy kategoriyalar ro'yxatini (admin tartibi/qo'shgan kategoriyalari bilan
   birga) tartiblangan holda qaytaradi. Har doim shu funksiya orqali
   foydalaniladi — DEFAULT_CATEGORIES to'g'ridan-to'g'ri ishlatilmaydi,
   chunki admin uni saytdan turib o'zgartirishi mumkin. */
function getAllCategories() {
  ...
  // Demo-rejim: localStorage override/custom mexanizmi (mahsulotlar kabi)
...
/* ---------------------------- Mahsulot baholari (yulduzchalar) ---------------------------- */
/* Har bir mahsulotga foydalanuvchilar 1-5 yulduzcha baho qo'yishlari
   mumkin. Struktura: { mahsulot_id: { telefon: 1..5 } } — Firebase
   yoqilgan bo'lsa umumiy (barcha foydalanuvchi ko'radi), aks holda faqat
   shu qurilmada saqlanadi. */
function ratingsFor(productId) {
  const source = FIREBASE_ENABLED ? (state.cloudRatings || {}) : state.ratings;
  return source[productId] || {};
}

function ratingSummary(productId) {
  const entries = Object.values(ratingsFor(productId));
...
function myRatingFor(productId) {
  if (!state.account) return 0;
  return ratingsFor(productId)[state.account.phone] || 0;
}

function submitRating(productId, stars) {
  if (!state.account) { showToast("Baho qo'yish uchun avval kiring"); return; }
  stars = Math.max(1, Math.min(5, Math.round(stars)));

  if (FIREBASE_ENABLED && fbDb) {
    fbDb.ref(`ratings/${productId}/${state.account.phone}`).set(stars);
    return;
  }
  state.ratings[productId] = { ...(state.ratings[productId] || {}), [state.account.phone]: stars };
  persist();
  render();
}

/* Yulduzchalarni HTML ko'rinishida chizadi. interactive=true bo'lsa,
   bosilganda submitRating chaqiriladi (mahsulot detali sahifasida). */
function starsHtml(productId, avg, interactive) {
  let out = `<div class="stars-row${interactive ? " interactive" : ""}" ${interactive ? `data-rate-product="${productId}"` : ""}>`;
  for (let i = 1; i <= 5; i++) {
    const filled = i <= Math.round(avg);
    out += `<span class="star${filled ? " filled" : ""}" ${interactive ? `data-star="${i}"` : ""}>★</span>`;
  }
  out += `</div>`;
  return out;
}

/* Lotin harflari, raqam va chiziqchadan tashkil topgan noyob "slug" (ID)
   yaratadi — yangi kategoriya qo'shilganda ishlatiladi. */
...
/* Kategoriyani bir pog'ona yuqoriga (dir=-1) yoki pastga (dir=1) suradi —
   order maydonlarini qo'shni kategoriya bilan almashtiradi. */
function moveCategory(slug, dir) {
  ...
  // Har bir kategoriyaga uning ro'yxatdagi joriy o'rnini order sifatida
  // qayta belgilaymiz, faqat ikkitasining o'rnini almashtirib.
...
/* Kategoriya maydonlarini (order/label/icon) demo-rejimda yangilaydi —
   standart yoki admin qo'shgan kategoriya bo'lishidan qat'iy nazar ishlaydi. */
...
function showToast(msg, duration) {
  ...
  // Uzunroq (masalan, xatolik tafsilotlari bo'lgan) xabarlar ko'proq vaqt
  // ko'rinib tursin — o'qib ulgurish uchun.
...
function showOnboardingFallback(reason) {
  const msg = document.getElementById("onboardStatusMsg");
  const fallback = document.getElementById("onboardFallback");
  const fallbackMsg = document.getElementById("onboardFallbackMsg");
  const retryBtn = document.getElementById("retryAuthBtn");
  if (msg) msg.style.display = "none";
  if (fallback) fallback.style.display = "block";

  if (reason === "timeout" || reason === "error") {
    // Telegram orqali kirilgan, lekin ma'lumot topilmadi/vaqt tugadi —
    // botga qaytish o'rniga "qayta urinish" ko'rsatamiz, chunki foydalanuvchi
    // odatda kontaktni allaqachon ulashgan bo'ladi (masalan, tarmoq sekin
    // bo'lgani uchun birinchi urinish muvaffaqiyatsiz tugagan bo'lishi mumkin).
    if (fallbackMsg) fallbackMsg.textContent = reason === "timeout"
      ? "Ma'lumotlaringizni tekshirish uzoq davom etyapti. Internetni tekshirib, qayta urinib ko'ring."
      : "Ma'lumotlaringiz topilmadi. Bot chatiga qaytib, kontaktingizni qayta ulashing yoki qayta urinib ko'ring.";
...
document.getElementById("retryAuthBtn").addEventListener("click", () => {
  document.getElementById("onboardStatusMsg").style.display = "block";
  document.getElementById("onboardFallback").style.display = "none";
  boot();
});
...
/* Telegram Mini App identifikatori (tg.initDataUnsafe.user.id) bo'yicha
   ...
   bayrog'i) o'qib, avtomatik kirishni yakunlaydi. 10 soniya ichida javob
   kelmasa (masalan, sekin tarmoq yoki Firebase ulanish muammosi), abadiy
   "tekshirilmoqda..." holatida qolib ketmasligi uchun vaqt chegarasi bor. */
function resolveTelegramAccount(tgUserId) {
  if (!FIREBASE_ENABLED || !fbDb) {
    console.error("Firebase sozlanmagan — Telegram orqali avtomatik kirish ishlamaydi.");
    showOnboardingFallback("error");
    return;
  }

  const readPromise = fbDb.ref("telegramContacts/" + tgUserId).once("value");
  const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("timeout")), 10000));

  Promise.race([readPromise, timeoutPromise])
    .then((snap) => {
      const data = snap.val();
      console.log("[Auth] telegramContacts/" + tgUserId, data);
      if (!data || !data.phone) {
        showOnboardingFallback("error");
        return;
      }
      const isTheAdmin = normalizePhone(data.phone) === ADMIN_PHONE_DIGITS;
      completeLogin(data.name || "Mijoz", data.phone, isTheAdmin);
    })
    .catch((err) => {
      const isTimeout = err && err.message === "timeout";
      console.error(isTimeout ? "Firebase javob berish vaqti tugadi" : "Telegram kontaktini o'qishda xatolik:", err);
      showOnboardingFallback(isTimeout ? "timeout" : "error");
    });
}

/* Tasdiqlangach akkaunt yaratish/kirish. isAdmin bayrog'i HAR DOIM shu
   chaqiruvda berilgan (Firebase'dan yangi o'qilgan) qiymatga teng
   qo'yiladi — avvalgi/keshlangan holat bilan HECH QACHON qo'shib (OR)
   hisoblanmaydi. Aks holda bir marta admin bo'lgan akkaunt (yoki shu
   qurilmada avval kirgan admin) doimiy ravishda admin bo'lib qolib,
   xavfsizlik teshigiga aylanardi. */
function completeLogin(name, phone, isTheAdmin) {
  const existing = state.knownAccounts[phone];

  state.account = existing
    ? { ...existing, name, isAdmin: isTheAdmin }
  ...
  saveJSON(LS.knownAccounts, state.knownAccounts);
  syncAccountToCloud(state.account);
  // MUHIM: bu yerda boot() emas — chunki boot() endi har doim Telegram
  // orqali qayta tekshiradi, va bu cheksiz tsikl hosil qilardi
  // (completeLogin -> boot -> resolveTelegramAccount -> completeLogin -> ...).
  // Endi shunchaki to'g'ridan-to'g'ri asosiy ekranga o'tamiz, chunki
  // yuqorida allaqachon yangi (Firebase'dan) tasdiqlangan holatdamiz.
...
/* Savat ekranida kabel summasi va umumiy jami to'lovni input maydonlarini
   qayta yaratmasdan (fokusni yo'qotmasdan) yangilaydi — shu funksiya
   set/tok kabel inputlariga har bir belgi kiritilganda chaqiriladi. */
    ...
    <div class="card" data-open="${p.id}">
      ...
      <div class="card-body">
        ...
        ${(() => {
          ...
          return count > 0 ? `<div class="card-rating">⭐ ${avg.toFixed(1)} <span>(${count})</span></div>` : "";
        })()}
...
function renderDetail(id) {
  ...
  return `
    ...
    ${(() => {
      const { avg, count } = ratingSummary(p.id);
      return count > 0
        ? `<div class="rating-summary">${starsHtml(p.id, avg, false)}<span class="rating-count">${avg.toFixed(1)} · ${count} ta baho</span></div>`
        : `<div class="rating-summary"><span class="rating-count">Hali baho yo'q</span></div>`;
    })()}
    ...
    <div class="rate-box">
      <div class="rate-box-title">Mahsulotni baholang</div>
      ${starsHtml(p.id, myRatingFor(p.id), true)}
    </div>
...
function attachDynamicListeners() {
  ...
  // MUHIM: bu yerda to'liq render() chaqirilmaydi — aks holda har bir
  // bosilgan tugmadan keyin input elementi qayta yaratilib, fokus (kursor)
  // undan uzilib qolar edi va faqat 1 ta raqam kiritish mumkin bo'lardi.
  // Shuning uchun faqat summalar DOM'da to'g'ridan-to'g'ri yangilanadi.
  ...
  document.querySelectorAll("[data-rate-product]").forEach((row) => {
    row.querySelectorAll("[data-star]").forEach((starEl) => {
      starEl.addEventListener("click", () => {
        const productId = row.dataset.rateProduct;
        const stars = parseInt(starEl.dataset.star, 10);
...
async function submitOrder(location, contactPhone) {
  ...
  try {
    ...
    // Avval umumiy "internetni tekshiring" xabari ko'rsatilar edi va
    // Telegram API'dan qaytgan HAQIQIY sabab (masalan: bot kanalga admin
    // qilib qo'shilmagan, token noto'g'ri, kanal ID xato) yashiringan edi.
    // Endi aniq sababni ko'rsatamiz — shu orqali muammoni tezroq topish mumkin.
...
function boot() {
  // XAVFSIZLIK: Telegram Mini App ichida bo'lsak, HAR DOIM joriy Telegram
  // foydalanuvchisi asosida Firebase'dan qayta tekshiramiz — localStorage'da
  // saqlangan eski akkaunt (va uning admin holati) hech qachon to'g'ridan-to'g'ri
  // ishonch manbai sifatida ishlatilmaydi. Aks holda, agar shu qurilmada
  // avval boshqa odam (masalan admin) kirgan bo'lsa, uning huquqlari
  // keyingi foydalanuvchiga "meros" bo'lib qolishi mumkin edi.
